            ...
            for j, surv in enumerate(survVals):
              ...
              #  n_neur_500_TP, nvals_500_TP, _aa_500_TP = thr_f.thr_function(TP_data[j], e_field, simParams)
                ...
                if plotfigs_1=='True':
                    ...
                    fig2.supxlabel('Longitudinal distance (mm)')
                    fig2.supylabel('Fractional neuronal activation')

                    fig2.tight_layout()
                    #plt.show()
            n_neur_results = np.array(n_neur_results)
            n_neur_offsets_results = np.array(n_neur_offsets_results)
...
#    print(f"{n_neur}", f"{n_neur_offsets_results}")
    return [n_neur_results, n_neur_offsets_results]

#if __name__ == '__main__':
# compare_neuron_count('main')  # alternatives are 'main', 'gui' and "survey'

subjects = ['S29']#, 'S22''S29', 'S40', 'S43', 'S47', 'S49R', 'S50']
empty_list = []
...
for s,subj in enumerate(subjects):
    ...
    datafile = "INV_OUTPUT/RE250_RI70std_2.0_thr_100/" + subj + "_fitResults_combined.csv"
    file = open(datafile)
    ...
    neuron_results, offsets_neuron_results = compare_neuron_count(subj)
#    full_neuron_results=np.column_stack((full_neuron_results,neuron_results[:,1]))
   # full_offset_neuron_results=np.column_stack((full_offset_neuron_results,offsets_neuron_results[:,1]))
